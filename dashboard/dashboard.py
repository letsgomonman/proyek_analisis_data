...
def create_revenue_category_df(df):
    revenue_per_category_df = df.groupby('product_category_name_english')['price'].sum().reset_index()
...
revenue_df = create_revenue_category_df(main_df)
rfm_df = create_rfm_df(main_df)

st.title('E-Commerce Dashboard :sparkles:')

st.subheader("Performa Pendapatan Kategori Produk")

fig, ax = plt.subplots(nrows=1, ncols=2, figsize=(24, 6))
colors = ["#72BCD4", "#D3D3D3", "#D3D3D3", "#D3D3D3", "#D3D3D3"]

if not revenue_df.empty:
    sns.barplot(x="price", y="product_category_name_english", data=revenue_df.head(5), palette=colors, ax=ax[0])
    ax[0].set_ylabel(None)
    ax[0].set_xlabel("Total Pendapatan (BRL)", fontsize=12)
    ax[0].set_title("Kategori Produk Pendapatan Tertinggi", loc="center", fontsize=15)
    ax[0].tick_params(axis='y', labelsize=12)

    sns.barplot(x="price", y="product_category_name_english", data=revenue_df.sort_values(by="price", ascending=True).head(5), palette=colors, ax=ax[1])
    ax[1].set_ylabel(None)
    ax[1].set_xlabel("Total Pendapatan (BRL)", fontsize=12)
    ax[1].invert_xaxis()
    ax[1].yaxis.set_label_position("right")
    ax[1].yaxis.tick_right()
    ax[1].set_title("Kategori Produk Pendapatan Terendah", loc="center", fontsize=15)
...
with st.expander("Lihat Insight Performa Produk"):
    st.write(
        """
         Berdasarkan analisis performa produk, kategori health_beauty secara konsisten menjadi penyumbang pendapatan (revenue) tertinggi bagi perusahaan. 
         Sebaliknya, kategori seperti cd_dvds_musicals menghasilkan pendapatan paling rendah, yang mengindikasikan kurangnya minat pasar di kategori tersebut.
        """
    )
